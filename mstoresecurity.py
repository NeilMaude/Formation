...
s_ModuleName = 'mstoresecurity.py'
...
def unit_test():
    n_pass = 0
    n_fail = 0
    print("\nRunning unit tests for " + s_ModuleName)
    print("\n" + str(n_pass + n_fail+1) + ": Create and retrieve credentials")
    user = 'Neil'
    ...
    if success:
        ...
        if success:
            if (name == user) and (password == password2):
                print('Test passed - credentials saved and retrieved.')
                n_pass += 1
            else:
                print('Credentials did not match... ', user, password, file, name, password2)
                n_fail += 1
        else:
    ...
    print("\nCompleted unit tests for " + s_ModuleName)
